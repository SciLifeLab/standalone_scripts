...
def backup(user, password, dest):
    ...
    for repo in chain(SL_repos, NGI_repos):
        ...
        if os.path.exists(repo_path):
            logger.info("The repository {} already exists on destination. Pulling " \
        ...
        #Otherwise clone the repository and fetch all branches
            ...
            logger.info("The repository {} isn't cloned at {}, cloning instead of updating...".format(repo.name, repo_path))
            try:
                check_call(['git', 'clone', source, repo_path], stdout=PIPE, stderr=STDOUT)
                logger.info("Cloning {}".format(repo.name))
            except CalledProcessError:
                logger.error('ERROR: Error cloning repository {}, skipping it'.format(repo.name))
            pass
            try:
                with cd(repo_path):
                ...
                logger.error('ERROR: Problem fetching branches for repository {}, skipping it'.format(repo.name))
                pass
...
if __name__=="__main__":
    ...
    parser.add_argument('--final_dest', type=str, help="Final destination of " \
                        "the copy, typically a directory that will be backed up",
                        required=True)
    ...
    logger.info("Creating backup at {}, with github user {}".format(dest, user))
